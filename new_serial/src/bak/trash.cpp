...
size_t write_data(char* buffer , size_t size, size_t nmemb, void* node)
{
    try {
        Json::Reader reader;
        Json::Value root;
        ...
        int isAutoTrash = getValueFromString(orderValue.c_str(), 1, 2);
        int leftServo = getValueFromString(orderValue.c_str(), 2, 4);
        int rightServo = getValueFromString(orderValue.c_str(), 4, 6);
        int motor = getValueFromString(orderValue.c_str(), 6, 8);
        int light = getValueFromString(orderValue.c_str(), 8, 10);
        ...
        ros::NodeHandle *mcuNode = static_cast<ros::NodeHandle*>(node);
