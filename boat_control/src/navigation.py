...
def gps_callback(msg):
    global Goal,heading,latitude,longitude,index,flag_gps,path_flag,pub1,cmd,path_now,auto_state,hedao_direction,qf,time_now,continue_flag,distance_to_current_goal
    if (index % 10 == 0):
    ...
    if(flag_gps == 1 and path_flag==1 and auto_state == 1):
        flag_gps = 0
        #print("dist2goal: ",distance_to_current_goal)
        move_close()
        ...
        if Goal==0:
            ...
            if qf == 4 and ( distance_flag == 1 or continue_flag == 1):
                ...
                print("hedao direction is :",hedao_direction)
                #print("move now")
...
def move_close():
    ...
    Goal = 0
    if distance_to_current_goal < 5:
        if len(path)>1:
            path=path[1:]
            path_now=path[0]
            distance_to_current_goal = 20
            print("now the path point is :",path)
        else:
            Goal=1
